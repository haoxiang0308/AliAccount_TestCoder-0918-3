...
# (?!\w)   : Negative lookahead to ensure the match is not followed by a word character (stops at boundary)
my @emails = $text =~ /[\w\.\-]+\@[\w\-]+(\.[\w\-]+)+(?!\w)/g;

print "Found emails: @emails\n";

# Write the found emails to a file with a random name
my $filename = "found_emails_v2_" . int(rand(10000)) . ".txt";
